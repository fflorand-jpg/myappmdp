...
import VaultPrint from './components/VaultPrint';
...
export default function App() {
  ...
  const handleExportBackup = async () => {
    const backupObj = {
      app: 'MDP Hors Ligne',
      exportsDate: new Date().toISOString(),
      canary,
    ...
    const isHybrid = (window as any).cordova || (window as any).Capacitor || (window as any).Android;
    const isWebView = isAndroid && (/Version\/\d+\.\d+/i.test(userAgent) || /wv/i.test(userAgent));
    const isMobileDevice = isAndroid || isIos || isStandalone || isHybrid || isWebView;

    const fileName = `mdp_chiffre_backup_${new Date().toISOString().slice(0, 10)}.json`;
    const jsonString = JSON.stringify(backupObj, null, 2);

    // 1. Try Native Share if on mobile device
      ...
      try {
        ...
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
          await navigator.share({
            files: [file],
            title: 'Sauvegarde de vos codes',
            text: 'Fichier de sauvegarde sécurisé et chiffré (.json) pour MDP Hors Ligne.'
          });
          return;
        }
      } catch (shareErr) {
    ...
    // 2. Try Standard File Download via Blob URL (extremely reliable on mobile & desktop browsers)
    ...
    try {
      ...
      console.error('Blob download failed:', downloadErr);
    }

    // 3. For mobile devices, copy to clipboard as a safe secondary backup
    if (isMobileDevice) {
      try {
        await navigator.clipboard.writeText(jsonString);
        showCustomAlert(
          'Sauvegarde de Sécurité',
          'Le fichier de sauvegarde (.json) a été généré pour le téléchargement. Vos données de sauvegarde chiffrées ont également été copiées dans le presse-papiers pour une sécurité maximale.'
        );
        return;
      } catch (clipErr) {
    ...
    if (!downloadSuccess) {
      ...
      try {
        ...
        console.error('All download methods failed', legacyErr);
      }
    }
  };
  ...
  const handleImportBackup = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    fileReader.onload = () => {
      try {
        ...
        if (!json.canary || !json.payload) {
          setImportError("Format de fichier invalide. Les champs de sécurité 'canary' ou 'payload' sont manquants.");
          return;
        }

        // Prompt user to verify password by typing it to see if it works with the imported file
        setPromptModal({
          isOpen: true,
          title: "Mot de passe Maître",
          message: "Veuillez entrer le mot de passe maître d'origine associé à cette sauvegarde pour valider et décoder l'importation :",
          placeholder: "Code d'accès...",
          isPassword: true,
          onConfirm: (pass) => {
            if (!pass) return;

            verifyCanary(json.canary, pass).then((isOk) => {
              if (!isOk) {
                setImportError("Le mot de passe maître d'origine saisi ne permet pas d'ouvrir ce fichier de sauvegarde.");
                setPromptModal(null);
                return;
              }

              // Import works! Save
              localStorage.setItem('vault_canary', json.canary);
              localStorage.setItem('vault_data', json.payload);
              setCanary(json.canary);
              setEncryptedDataPayload(json.payload);
              setIsConfigured(true);
              
              // Decrypt live
              decryptData(json.payload, pass).then((decStr) => {
                const parsed = JSON.parse(decStr) as PasswordEntry[];
                setMasterPassword(pass);
                setEntries(parsed);
                setIsLocked(false);
                setImportSuccess(true);

                // Save recovery payload too
                encryptData(decStr, '&').then((recoveryEncryptedPayload) => {
                  localStorage.setItem('vault_data_recovery', recoveryEncryptedPayload);
                }).catch(e => console.warn('Could not encrypt recovery payload on import', e));
        ...
        setImportError("Erreur lors de la lecture du fichier JSON. Assurez-vous d'avoir choisi un fichier correct.");
      }
    };
  ...
  if (showPrintMode) {
    return <VaultPrint entries={entries} categories={categories} onClose={() => setShowPrintMode(false)} />;
  }
          ...
          <div className="bg-white/95 backdrop-blur-md border border-white/20 p-8 rounded-3xl w-full max-w-md shadow-2xl space-y-6 transform hover:scale-[1.01] transition-all duration-300">
            ...
            <form onSubmit={handleSetup} className="space-y-4">
              ...
              <div className="pt-2 space-y-3">
                ...
                <div className="relative flex py-1 items-center text-xs text-neutral-400 font-medium select-none">
                  <div className="flex-grow border-t border-neutral-200"></div>
                  <span className="flex-shrink mx-3 text-neutral-400 font-mono text-[9px] font-bold">OU</span>
                  <div className="flex-grow border-t border-neutral-200"></div>
                </div>
                ...
                <button
                  ...
                  className="w-full bg-neutral-50 hover:bg-neutral-100 text-neutral-700 hover:text-neutral-950 border border-neutral-200 hover:border-neutral-300 font-semibold py-2 px-3 rounded-xl text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button
                  ...
                  className="bg-rose-50 hover:bg-rose-100 border border-rose-200 text-rose-700 p-2.5 rounded-xl transition-all flex items-center justify-center cursor-pointer shadow-2xs hover:scale-[1.03] active:scale-95 gap-1.5 font-semibold text-xs px-3"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-neutral-900/40 backdrop-blur-xs">
          <div className="bg-white border border-neutral-200 rounded-xl w-full max-w-md shadow-xl p-6">
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-neutral-100">
              <h3 className="font-semibold text-neutral-900 text-sm flex items-center gap-1.5 font-sans tracking-tight">
                <Upload size={16} className="text-neutral-700" /> Restaurer une sauvegarde (.json)
              </h3>
              <button
                onClick={() => {
            ...
            <div className="space-y-4">
              <p className="text-xs text-neutral-500 leading-relaxed">
                Choisissez le fichier JSON de sauvegarde précédemment exporté depuis cet outil. L'application décodera vos comptes après avoir vérifié le mot de passe maître correspondant à ce fichier.
              </p>

              <div className="border border-dashed border-neutral-300 hover:border-neutral-400 bg-neutral-50 p-6 rounded-lg text-center cursor-pointer transition-colors relative group">
                <input
                  type="file"
                  accept=".json"
                ...
                />
                <Download size={24} className="mx-auto text-neutral-400 mb-2 group-hover:text-neutral-800 transition-colors" />
                <span className="block text-xs font-semibold text-neutral-700">Sélectionner le fichier de sauvegarde</span>
                <span className="block text-[10px] text-neutral-400 mt-1">Fichier .json uniquement</span>
              </div>

              {importError && (
                <div className="bg-red-50 border border-red-200 p-3 rounded text-red-850 text-xs text-center font-medium">
                  {importError}
                </div>
              )}

              {importSuccess && (
                <div className="bg-neutral-900 text-white p-3 rounded text-xs text-center font-semibold">
                  ✓ Restauration réussie. MDP déverrouillé !
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white border border-neutral-200 rounded-2xl w-full max-w-md shadow-xl p-6 space-y-5">
            ...
            <div className="space-y-4">
              ...
              <div className="pt-3 border-t border-neutral-100 space-y-2">
                <label className="block text-xs font-semibold text-neutral-800">
                  Sauvegardes locals (.json)
                </label>
                <p className="text-[11px] text-neutral-500">
                  Toutes vos données résident uniquement dans ce navigateur. Exportez régulièrement pour ne pas les perdre.
                </p>
                <div className="space-y-1.5 pt-1">
                  <button
                    type="button"
                    onClick={handleExportBackup}
                    className="w-full bg-neutral-50 border border-neutral-200 hover:border-neutral-300 text-neutral-700 font-medium py-2 px-3 rounded-lg text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="w-full bg-neutral-50 border border-neutral-200 hover:border-neutral-300 text-neutral-700 font-medium py-2 px-3 rounded-lg text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                  >
                    <Upload size={14} /> Importer sauvegarde (.json)
                  </button>
                </div>
                ...
                <button
                  ...
                  className="w-full bg-neutral-50 border border-neutral-200 hover:border-neutral-300 text-neutral-700 font-medium py-2 px-3 rounded-lg text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                >
                  <FileText size={14} /> Exporter en fichier PDF
                </button>
              </div>
