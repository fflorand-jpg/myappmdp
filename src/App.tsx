  ...
  const handleExportBackup = async () => {
    ...
    if (isMobileDevice && navigator.share && typeof File !== 'undefined') {
      try {
        ...
        console.warn('Share API failed, falling back to copy/download:', shareErr);
      }
    }

    // Secondary fallback for mobile devices / APK if sharing fails or isn't supported
    if (isMobileDevice) {
      try {
        await navigator.clipboard.writeText(jsonString);
        showCustomAlert(
          'Sauvegarde Copiée !',
          'Les téléchargements directs de fichiers sont limités sur cette plateforme mobile APK. Vos données de sauvegarde JSON chiffrées ont été copiées dans votre presse-papier. Vous pouvez les coller et les enregistrer en toute sécurité.'
        );
        return;
      } catch (clipErr) {
    ...
    // Default web browser download behavior
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button
                  ...
                  className="bg-white border border-neutral-200 hover:bg-neutral-50 p-2.5 text-neutral-600 hover:text-neutral-900 rounded-xl transition-all flex items-center justify-center cursor-pointer shadow-2xs"
