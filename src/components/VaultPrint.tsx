...
export default function VaultPrint({ entries, onClose, categories }: VaultPrintProps) {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      // Detect if running on mobile/APK to use Share sheet (since downloads are blocked on some mobile environments/WebView APKs)
      ...
      if (isMobileDevice && navigator.share && typeof File !== 'undefined') {
        try {
          ...
          const file = new File([pdfBlob], 'sauvegarde_codes.pdf', { type: 'application/pdf' });
          if (navigator.canShare && navigator.canShare({ files: [file] })) {
            await navigator.share({
              files: [file],
              title: 'Sauvegarde Codes PDF',
              text: 'Export sécurisé de mes codes d\'accès.'
            });
          } else {
